...
- `SENSORARRAY_APP_MODE_PIEZO_READ`: 默认，运行 `sensorarrayRunPiezoRead()`，`SW=REF`。
- `SENSORARRAY_APP_MODE_RESISTANCE_READ`: 运行 `sensorarrayRunResistanceRead()`，`SW=GND`。
...
6. apply selected SW source，`PIEZO_READ -> REF`，`RESISTANCE_READ -> GND`
...
```text
APPMODE,active=RESISTANCE_READ,cnName=电阻读取,skipAdsInit=0,skipFdcInit=1,sw=GND
DBGROUTEPOLICY,mode=RESISTANCE_READ,sw=GND,sela=ADS126X,fdcInitSkipped=1
```
