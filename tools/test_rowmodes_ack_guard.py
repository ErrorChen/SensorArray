#!/usr/bin/env python3
"""Compile and execute the firmware's ROWMODES ACK/terminal guard test."""
...
def main() -> int:
    ...
    source = root / "tests" / "host" / "test_rowmodes_ack_guard.c"
    with tempfile.TemporaryDirectory(prefix="sensorarray-rowmodes-ack-test-") as tempDir:
        temp = Path(tempDir)
        fakeDir = temp / "fakes"
        fakeDir.mkdir()
        ...
        if not native:
            subprocess.run(
                common + ["-c", str(source), "-o", str(temp / "rowmodes-ack.o")],
                check=True,
                env=compilerEnvironment,
            )
            print("ROWMODES_ACK_GUARD_COMPILE,passed=1,target=embedded,execution=skipped")
            return 0
        executable = temp / "test_rowmodes_ack_guard.exe"
        subprocess.run(
