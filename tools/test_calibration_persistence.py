#!/usr/bin/env python3
"""Compile and run the calibration persistence C tests on the host."""
...
def main() -> int:
    ...
    sources = [
        root / "tests" / "host" / "test_calibration_persistence.c",
        root / "core" / "config" / "sensorarrayCalibration.c",
    ]
    with tempfile.TemporaryDirectory(
        prefix="sensorarray-calibration-test-"
    ) as temp_dir:
        common = [
            compiler,
            "-std=c11",
        ...
        if not native:
            for index, source in enumerate(sources):
                subprocess.run(common + ["-c", str(source), "-o",
                                         str(Path(temp_dir) / f"calib-{index}.o")],
                               check=True, env=compiler_environment)
            print("CALIBRATION_PERSISTENCE_COMPILE,passed=1,target=embedded,execution=skipped")
            return 0
        executable = Path(temp_dir) / "test_calibration_persistence.exe"
